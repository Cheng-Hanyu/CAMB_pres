...
!chy
        real(dl) :: Omega1_chy = 0._dl  ! Omega1 parameter
!chy
...
!chy
    function w_de(this, grhornomass, grhog, grhoc, grhob, grhonu_mio, grhok, grhocrit, a)
!chy
    class(TDarkEnergyModel) :: this
    real(dl) :: w_de, al
    real(dl), intent(IN) :: a
!chy
    real(dl), intent(in) :: grhornomass, grhog, grhoc, grhob, grhonu_mio, grhok, grhocrit
!chy
...
!chy
    function grho_de(this, grhornomass, grhog, grhoc, grhob, grhonu_mio, grhok, grhocrit, a)  !relative density (8 pi G a^4 rho_de /grhov)
!chy
    class(TDarkEnergyModel) :: this
...
!chy
    subroutine BackgroundDensityAndPressure(this, grhov, a, grhov_t, w, grhornomass, grhog, grhoc, grhob, grhonu_mio, grhok, grhocrit)
...
!chy
    ...
    if (this%is_cosmological_constant) then
        ...
        if (a > 1e-10) then
!chy
            grhov_t = grhov * this%grho_de(grhornomass, grhog, grhoc, grhob, grhonu_mio, grhok, grhocrit, a) / (a * a)
            !grhov_t = grhov * this%grho_de(a) / (a * a)
!chy
        else
            grhov_t = 0._dl
        end if
        if (present(w)) w = this%w_de(grhornomass, grhog, grhoc, grhob, grhonu_mio, grhok, grhocrit, a)
    end if
...
!chy
    function TDarkEnergyEqnOfState_w_de(this, grhornomass, grhog, grhoc, grhob, grhonu_mio, grhok, grhocrit, a)
!chy
    class(TDarkEnergyEqnOfState) :: this
...
!chy
    real(dl), intent(in) :: grhornomass, grhog, grhoc, grhob, grhonu_mio, grhok, grhocrit
    real(dl) :: Omega_matter, Omega_rad, Omega_curv, OmegaDE0_chy
!chy

    if(.not. this%use_tabulated_w) then
!chy
        Omega_matter = (grhoc+grhob+grhonu_mio)/grhocrit
        Omega_rad = (grhornomass + grhog)/grhocrit
        Omega_curv = grhok/grhocrit
        OmegaDE0_chy = 1 - Omega_matter - Omega_curv - Omega_rad

        if (this%Omega1_chy >= OmegaDE0_chy) then
            if (abs(a - (1._dl - OmegaDE0_chy/this%Omega1_chy)) < 1.e-10_dl) then
                TDarkEnergyEqnOfState_w_de = -1._dl
            else
                TDarkEnergyEqnOfState_w_de = -1._dl + (1._dl/3._dl) * (this%Omega1_chy * a) / &
                    &(this%Omega1_chy * (1._dl - a) - OmegaDE0_chy)
            endif
        else
            TDarkEnergyEqnOfState_w_de = -1._dl + (1._dl/3._dl) * (this%Omega1_chy * a) / &
                &(this%Omega1_chy * (1._dl - a) - OmegaDE0_chy)
...
!        if (this%Omega1_chy >= OmegaDE0_chy) then
!            TDarkEnergyEqnOfState_w_de = -10._dl
!        else
!            TDarkEnergyEqnOfState_w_de = -1._dl + (1._dl/3._dl) * (this%Omega1_chy * a) / (this%Omega1_chy * (1._dl - a) - OmegaDE0_chy)
!        endif
!       print *, a, TDarkEnergyEqnOfState_w_de
!chy
    else
        al=dlog(a)
    ...
    function TDarkEnergyEqnOfState_grho_de(this, grhornomass, grhog, grhoc, grhob, grhonu_mio, grhok, grhocrit, a) result(grho_de) !relative density (8 pi G a^4 rho_de /grhov)
    class(TDarkEnergyEqnOfState) :: this
    real(dl) :: grho_de, al, fint
    real(dl), intent(IN) :: a
!chy
...
!        if (this%Omega1_chy >= OmegaDE0_chy) then
!            grho_de = -a**4._dl * 0._dl
!        else
        grho_de = a**4._dl * (1._dl + (a - 1._dl) * this%Omega1_chy / OmegaDE0_chy)
!        endif 
!chy
    else
        if(a == 0.d0)then
...
!chy
        write(*,'("Omega1 = ", f8.5)') this%Omega1_chy
!chy
    endif
    ...
    if(.not. this%use_tabulated_w)then
!chy
        this%Omega1_chy = Ini%Read_Double('Omega1_chy', 0._dl)
!chy
