# CAMB with First-Order Dark Energy Pressure Parametrization

This modified version of [CAMB](https://github.com/cmbant/CAMB) implements
the first-order dark energy pressure expansion studied in
...
At first order (Eqs. 2.3–2.6),

```math
\rho=\rho_{\mathrm{DE},0}-\frac34(1-a)p_1,
```

```math
p=-\rho_{\mathrm{DE},0}+\left(\frac34-a\right)p_1,
\qquad \Omega_1\equiv\frac34\frac{p_1}{\rho_{\mathrm{crit}}},
```

```math
w_{\mathrm{DE}}=-1+\frac13
\frac{\Omega_1a}{\Omega_1(1-a)-\Omega_{\mathrm{DE},0}}.
```

Setting $\Omega_1=0$ recovers a cosmological constant. This is an expansion of
the pressure $p(a)$ about $a=1$, not a CPL expansion of $w_{\mathrm{DE}}$.
...
pressure-expansion coefficient. The analytic formula applies when
